...
https://github.com/bpiel/guildsman[Guildsman]
...
about whether this development session is complete.
...
fitness score. _"This trained model correctly identifies cats 90% of
the time. Let's ship it!"_ This fitness score is the result of testing
...
tests. There's all sorts of organizational questions related to that:
...
direction, but it is far from addressing everything above.
...
The goal is a scenario where you can drop a package identifier as an
input to your graph and Guildsman will be able to:
...
repos. For more on that dream, check out https://qri.io/.
...
with Guildsman's workflow feature.
...
some kind. The data structure gets _compiled_ to Clojure code and then
...
Why do we need plugins? I only have two well-defined ideas about what
plugins will be good for, but they're both pretty big deals.
...
As training occurs, TensorFlow automatically and periodically saves
the state of variables into files called "checkpoints".
...
In the future, I'd like to add support for pushing checkpoints and
...
checkpoint repo could serve as a source for deployment as well.

'''

As always, I expect to have an alpha release out in the next few weeks.

If you'd like to read more, or get in touch, check out the
https://github.com/bpiel/guildsman[Guildsman README].
